...
// FindScsiHostsForTarget given a target iqn, find all the scsi hosts logged into that target.
func (handler *deviceHandler) FindScsiHostsForTarget(targetIqn string, portals []string) (map[int]int, error) {
	...
	for _, hostDir := range hostDirs {
		...
		for _, deviceDir := range deviceDirs {
			...
			if err != nil {
				glog.Error("Could not get number from iISCSI session: %s", sessionName)
				continue
			}
...
func (handler *deviceHandler) FindDevicesForLun(targetIqn string, portals []string, lun int) ([]string, error) {
	...
	for _, hostDir := range hostDirs {
		...
		for _, deviceDir := range deviceDirs {
			...
			// We assume the channel and bus are always zero for iSCSI
